...
- The paper is retitled *World Models for Cross-Machine CNC Transfer under Partial Sensor Overlap*
  (arXiv v2 of 2609.16071, announced @V2_DATE@), after Jean Ponce's review: the method acronym
  ...
  "World model (locked, M03)", `docs/paper.pdf` is the v2 build and `docs/og.png` shows the new
  title. arXiv derives the BibTeX key from the title, so the key changes from
  `bouaziz2026schemaadaptiveactionconditionedjepacrossmachine` to `@NEW_KEY@`, copied from the v2
  abs page. The identifier, the DOI `10.48550/arXiv.2609.16071` and the abs URL do not change:
