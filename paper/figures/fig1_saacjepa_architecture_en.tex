...
\node[nrm] (n1) at (3.45,-1.2) {RevIN\\(variant)};
...
\node[act, minimum width=24mm] (fact) at (11.6,-2.6) {future actions $a_{T:T+h}$};
...
\node[nrm] (n2) at (3.45,-6.1) {RevIN\\(variant)};
...
\node[sub, text=mnormL, align=left, below=6mm of loss, anchor=north]
  {dashed green: post-lock RevIN variant (App.~H) ---\\
   context-window stats on both inputs,\\
   inverse on the physical head; not in the locked model};
