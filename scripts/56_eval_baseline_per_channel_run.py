...
p=argparse.ArgumentParser(); p.add_argument('--config',default='configs/base.yaml'); p.add_argument('--ckpt',required=True); p.add_argument('--model',required=True); p.add_argument('--device',default='auto'); p.add_argument('--out',default='outputs/baseline_per_channel_run.csv'); a=p.parse_args(); c=load_config(a.config); _,_,ds,sn,_=prepare(c,training_mask=False); ld=loaders_from_ds(ds,c,False); dev=device_from_arg(a.device); m=build_forecaster(a.model,c); m.load_state_dict(torch_load_checkpoint(a.ckpt,map_location='cpu')['model']); m.to(dev); _,raw=evaluate_forecaster(m,ld['target_all'],dev); y,pred,mask=raw['y'],raw['pred'],raw['mask']; yp=sn.inverse(y); pp=sn.inverse(pred); rows=[]
