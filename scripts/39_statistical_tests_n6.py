...
p=argparse.ArgumentParser(); p.add_argument('--jepa',default='outputs/per_channel_run.csv'); p.add_argument('--baseline'); p.add_argument('--out',default='outputs/paired_stats_n6.json'); a=p.parse_args(); j=pd.read_csv(a.jepa); jrun=j.groupby('run').rmse_z.mean();
if a.baseline:
