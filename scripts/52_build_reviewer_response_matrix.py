...
['Per-horizon vs pooled VICReg','16','none/pooled/per-horizon anti-collapse comparison.'],
['With/without LayerNorm','17','Direct latent-space LayerNorm on/off plus separate Transformer pre/post norm ablation.'],
...
['Action conditioning validity','13,37,38','No-action/shuffled-action controls and world-model planning experiment.'],
['Random method search can leak target','40-45','20 unique methods x 3 seeds selected on source validation only. Best method is hashed/locked before the first DS03 test.'],
['Need a defensible novelty','12,37,46-50','Schema-Adaptive Action-Conditioned JEPA: cross-machine partial-schema dynamics, action identifiability, few-shot/online adaptation and planning utility.']]
pd.DataFrame(rows,columns=['reviewer_point','scripts','response']).to_csv(a.out,index=False); print(a.out)
