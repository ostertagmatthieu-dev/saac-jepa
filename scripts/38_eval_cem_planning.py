...
p=argparse.ArgumentParser(); p.add_argument('--config',default='configs/base.yaml'); p.add_argument('--ckpt',default='outputs/jepa_finetune/best.pt'); p.add_argument('--device',default='auto'); p.add_argument('--n',type=int,default=10); p.add_argument('--out',default='outputs/cem_plans.json'); a=p.parse_args(); c=load_config(a.config); _,sp,ds,sn,an=prepare(c,training_mask=False); ld=loaders_from_ds(ds,c,False)['target_all']; dev=device_from_arg(a.device); m=build_jepa(c,True); m.load_state_dict(torch_load_checkpoint(a.ckpt,map_location='cpu')['model'],strict=False); m.to(dev).eval(); acts=sp['source_train'][c['schema']['actions']].to_numpy(float); lo=an.transform(np.nanpercentile(acts,1,axis=0)); hi=an.transform(np.nanpercentile(acts,99,axis=0)); rows=[]
for b in ld:
 b=batch_to(b,dev)
