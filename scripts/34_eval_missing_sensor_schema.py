...
p=argparse.ArgumentParser(); p.add_argument('--config',default='configs/base.yaml'); p.add_argument('--ckpt',default='outputs/jepa_finetune/best.pt'); p.add_argument('--device',default='auto'); p.add_argument('--out',default='outputs/schema_sensitivity.json'); a=p.parse_args(); c=load_config(a.config); dev=device_from_arg(a.device); model=build_jepa(c,True); model.load_state_dict(torch_load_checkpoint(a.ckpt,map_location='cpu')['model'],strict=False); model.to(dev); rep={}; sensors=c['schema']['sensors']; transfer=c['schema']['transfer_sensors']
